  ...
  return new Promise(function (resolve, reject) {
    ...
    const req = clientModule.request(url, function (response) {
      if (response.statusCode !== 200) {
        ...
        reject(
          new Error(
            'Unable to fetch ' + url + ' - HTTP Status ' + response.statusCode
          )
        );
      } else {
        resolve(response);
    ...
    req.on('error', function (error) {
      // E.g., DNS resolution errors.
      reject(new Error('Unable to fetch ' + url + ' - ' + error.message));
    });
    req.end();
...
function Validator(scriptContents) {
  ...
  try {
    ...
    throw new Error(
      'Could not instantiate validator_wasm.js - ' + error.message
    );
  }
}
...
Validator.prototype.validateString = function (inputString, htmlFormat) {
  const internalResult = this.sandbox.amp.validator.validateString(
    inputString,
    htmlFormat
  );
...
function getInstance(opt_validatorJs, opt_userAgent) {
  ...
  return validatorJsPromise
    .then(function (scriptContents) {
      let instance;
      try {
...
function main() {
  ...
  if (
    opts.html_format !== 'AMP' &&
    opts.html_format !== 'AMP4ADS' &&
  ...
  getInstance(opts.validator_js, opts.userAgent)
    ...
    .catch(function (error) {
      process.stderr.write(
        (opts.format == 'color' ? colors.red(error.message) : error.message) +
          '\n',
        function () {
          process.exit(1);
        }
