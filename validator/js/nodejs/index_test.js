...
it('built validator rejects the empty file', function (done) {
  ...
  ampValidator
    .getInstance(/*validatorJs*/ '../../dist/validator_minified.js')
    .then(function (instance) {
      const validationResult = instance.validateString('');
...
it('rejects a specific file that is known to have errors', function (done) {
  // Note: This will use the validator that was built with build.py.
  const severalErrorsHtml = fs
    .readFileSync('../../testdata/feature_tests/several_errors.html', 'utf-8')
    .trim();
  const severalErrorsOut = fs
    .readFileSync('../../testdata/feature_tests/several_errors.out', 'utf-8')
    .split('\n')
    .filter(isErrorLine)
    .join('\n');
...
it('handles syntax errors in validator file', function (done) {
  ...
  ampValidator
    .getInstance(/*validatorJs*/ '../../dist/validator.protoascii')
    .then(function (instance) {
...
it('also works with newInstance', function () {
  const mini = fs
    ...
    .trim();
  const validatorJsContents = fs.readFileSync(
    '../../dist/validator_minified.js',
    'utf-8'
  );
  const resultForMini = ampValidator
    .newInstance(validatorJsContents)
    .validateString(mini);
  ...
  const severalErrorsHtml = fs
    .readFileSync('../../testdata/feature_tests/several_errors.html', 'utf-8')
    .trim();
  const resultForSeveralErrors = ampValidator
    .newInstance(validatorJsContents)
...
it('emits json if --format=json is specified on command line', function (done) {
  execFile(
    ...
    function (error, stdout, stderr) {
      ...
      expect(parsedJson['feature_tests/minimum_valid_amp.html']).toBeDefined();

      expect(parsedJson['feature_tests/several_errors.html']).toBeDefined();
      expect(parsedJson['feature_tests/minimum_valid_amp.html'].status).toBe(
        'PASS'
      );

      expect(parsedJson['feature_tests/several_errors.html'].status).toBe(
        'FAIL'
      );
      done();
    }
