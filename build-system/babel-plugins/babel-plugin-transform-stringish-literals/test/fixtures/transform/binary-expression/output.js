...
let numberEnd = 'foo/1';
let stringEnd = 'foo/1';
let illegalCharacterString =
  'Invalid share providers configuration for story. Value must be `true` or a params object.';
let illegalCharacterTemplate = `Invalid ${x}Value must be \`true\` or a params object.`;
let illegalEscapeValue = `Invalid ${x}\${foo}`;

inverted: {
  let illegalCharacterString =
    'Value must be `true` or a params object. Invalid share providers configuration for story.';
