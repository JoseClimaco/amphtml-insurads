      ...
      onKeyDown: function (e, buffer, caretPos, opts, initial, previousPos) {
        ...
        if (
          ...
          if (length > 0 && cursorIsInMiddle) {
            ...
            if (e.keyCode === Inputmask.keyCode.BACKSPACE) {
              const selectionBegin = difference == 0 ? begin - 1 : begin;
              input.value =
                initial.slice(0, selectionBegin) + initial.slice(end);
              input.setSelectionRange(selectionBegin, selectionBegin);
            } else if (e.keyCode === Inputmask.keyCode.DELETE) {
              // delete
              const selectionEnd = difference == 0 ? end + 1 : end;
              input.value =
                initial.slice(0, begin) + initial.slice(selectionEnd);
        ...
        if (previousPos.begin != previousPos.end) {
          ...
          if (!/\s/.test(char)) {
            ...
            // set the value here.
            setTimeout(() => {
              input.value = initial.slice(0, begin) + char + initial.slice(end);
              input.setSelectionRange(begin + 1, begin + 1);
            }, 0);
          }
        }
      },
